...
public function ThisMonth(){
  $month = date('F');
  $order = DB::table('orders')->where('status',4)->where('month',$month)->get();
...
public function Search(){
 ...
 public function SearchByYear(Request $request){
 	...
 	$total = DB::table('orders')->where('status',4)->where('year',$year)->sum('total');

 	$order = DB::table('orders')->where('status',4)->where('year',$year)->get();
 ...
 public function SearchByMonth(Request $request){
 	...
 	$total = DB::table('orders')->where('status',4)->where('month',$month)->sum('total');
 ...
 public function SearchByDate(Request $request){
 	...
 	$total = DB::table('orders')->where('status',4)->where('date',$newdate)->sum('total');

 	$order = DB::table('orders')->where('status',4)->where('date',$newdate)->get();
